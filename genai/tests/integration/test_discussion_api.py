...
class TestDiscussionAPI(unittest.TestCase):
    ...
    def test_start_discussion_and_flow(self):
        ...
        self.assertEqual(discussion_data["topic"], "Career Guidance")
        ...
        self.assertEqual(response.status_code, 200, f"Get details failed: {response.text}")
        details = response.json()
        self.assertEqual(details["topic"], "Career Guidance")
        self.assertTrue(len(details["cards_drawn"]) > 0)
        self.assertTrue(isinstance(details["history"], list))

        # Get user discussions list
        response = requests.get(f"{BASE_URL}/discussions/test_user_123")
        self.assertEqual(response.status_code, 200, f"Get user discussions failed: {response.text}")
        discussions = response.json()
        self.assertTrue("discussions" in discussions)
        self.assertTrue(any(disc["discussion_id"] == discussion_id for disc in discussions["discussions"]))
