...
def load_tarot_with_history_template(path: str = "app/tarot_prompt_with_history_template.txt") -> str:
    """Load template for tarot reading with conversation history"""
    with open(path, "r", encoding="utf-8") as f:
        return f.read()
...
def render_prompt_with_history(
    ...
    """Render prompt with conversation history using your existing card rendering logic"""
    cards_lines = []
    for card, upright, meaning, pos, position_keywords in picks:
        orient = "Upright" if upright else "Reversed"
        ...
        for attr in ['archetype', 'hebrew_alphabet', 'numerology', 'elemental', 'mythical_spiritual']:
            if hasattr(card, attr) and getattr(card, attr):
                card_info.append(f"  {attr.replace('_', ' ').title()}: {getattr(card, attr)}")
